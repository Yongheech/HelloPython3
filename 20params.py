...
#  함수 호출할 때, 이 매개변수에 값을 전달하면
#  함수내에서 해당 값을 사용할 수 있음
#  즉, 매개변수는 입력된 데이터를 함수에 제공하는 역할 담당

#  가변인수
#  함수 호출시 인수의 갯수가 가변적일 수 있게 하는 기능
#  이를 통해 필요한 만큼의 인수를 자유롭게 전달 가능
#  파이썬에서는 *args, **kwargs 라는 구문사용
#  단, *args → 튜플,  **kwarg → dict 형태로 전달됨
...
print_kwargs(name='일지매', kor=99, eng=98, met=99)

#언패킹
# 시퀸스 자료형이나 dict 등의 값을 개별변수에 할당하는 과정
...
print_args2(**mtdict) # dict의 값들을 개별적으로 출력

# 매개변수 기본값
# 함수 정의시 매개변수의 기본값을 지정하면
# 함수 호출시 매개변수의 인수가 할당되지 않을 경우,
