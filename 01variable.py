# 변수 variable
# 어떤 값을 저장하는 장소(메모리)를 기억하기 쉽게 문자로 나타낸것
# 변수에 값이 저장되는 공간은 메모리라고 함
# 변수에 값을 넣어라고 선언하면,
# 시스템상의 메모리 어딘가에 공간을 확보하고
# 그 공간에 실제주소와 이름을 연관mapping함
...
# 자료형 datatype
# 정적static 자료형
# 변수의 자료형도 함께 지정 - 컴파일시간에 결정
# 자료형 검사가 컴파일 시간에 결정 - 오류발생 적음
# String name =  '홍길동'
#        name = 123 (오류발생!- 선언시 자료형과 대입시 값이 다름)

# 동적dynamic 자료형
# 변수선언시 자료형은 생략가능, 추론기능으로 자동할당
# 변수에 자료형은 실행 시간에 결정 - 오류발생 여지 존재
...
print(userid,passwd,name,age,email,married,money,regdate)
...
int3 = 0b00001000 # 2진수 : 0b (bin)~
int4 = 0x000FFFF # 16진수 : 0x (hex)~

print(int1, int2, int3, int4, hex(65535), bin(65535))

# 실수형 - 소수점이하 값이 있는 숫자
# 부동소수점floationg point 실수 : 국제표준에 따라 실수를 표기하는 방식
...
# 논리형
# 논리적인 값을 표현할 때 사용 - 참True(1), 거짓False(0)
...
# 명시적explicit 변환 - 개발자가 직접 변환 (int, str, flort, bool)

# 암시적 변환
result1 = 5+ 3.14152 # 정수 + 실수 조금 더 큰 자료형으로 변환
...
# print(type(str1), int(str1))#  실행 불가 !
